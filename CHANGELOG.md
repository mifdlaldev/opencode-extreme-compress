...
- 3-layer adaptive compression architecture
  ...
  - Layer 2: File content stripper (regex-based comment removal)
  - Layer 3: Semantic summarizer (LLM-based, wired via `opencode.client.session.prompt()`)
...
- Output budget tracking (triggers compression when budget low)
...
- Plugin signature accepts `PluginInput` for proper SDK access
- GitHub Actions CI (type check + tests + build verification)
...
### Test Coverage
- 131 tests across 13 files
- 15 hallucination resistance tests (BLOCKING — release blocked if any fails)
- 5 integration tests
- 5 model-specific behavior tests

### Known Limitations
- Layer 3 requires an active LLM model; in shadow/light modes this layer is dormant
